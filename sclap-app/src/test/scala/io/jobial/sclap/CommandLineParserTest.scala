  ...
  "exception stack trace" should behave like {
    val spec = command.printStackTraceOnException(true) {
      for {
        ...
        IO.raiseError[Unit](new Error("an error"))
    }

    runCommandLineTestCases(spec)(
      Seq() -> failWith[Unit](t => assert(t.getMessage == "an error"),
        out = {out: String => assert(out == "")}, err = {err: String => assert(err.contains("at io.jobial.sclap"))}),
      Seq("-a", "hello") -> failWith[Unit](t => assert(t.getMessage == "an error"),
        out = {out: String => assert(out == "")}, err = {err: String => assert(err.contains("at io.jobial.sclap"))})
    )
  }
...
""")
  ...
  "parsing param range test" should behave like {
    val spec =
      for {
        a <- params[String].fromIndex(0) //.toIndex(5)
      } yield IO {
        a
      }

    runCommandLineTestCases(spec)(
  //    Seq() -> succeedWith(List()),
      Seq("1", "2") -> succeedWith(List("1", "2"))
    )
  }
...
""")
    ...
    "errors in subcommands" should behave like {
      val subcommand1 = subcommand("s1") {
        for {
          a <- opt[String]("--a")
        } yield
          if (a eqv Some("0")) {
      ...
      val subcommand4 = subcommand("s4") {
        for {
          b <- opt[Int]("-b").default(1)
        } yield {
          IO.delay(println("s4 running")) >> {
            if (b eqv 0)
              IO.raiseError(new IllegalArgumentException("s4 failed"))
            else if (b eqv 98)
              IO.raiseError(new IncorrectCommandLineUsage("b cannot be 98"))
            else
              IO(b)
          }
        }
      ...
      val subcommand2 = subcommand("s2", "s3") {
        for {
          b <- opt[Int]("-b").default(1)
          s4 <- subcommands(subcommand4)
        } yield s4 orElse {
          IO(println("s2 running")) >> {
            if (b eqv 0)
              IO.raiseError(new IllegalArgumentException("s2 failed"))
            else if (b eqv 99)
              IO.raiseError(new IncorrectCommandLineUsage("b cannot be 99"))
...
//          s1 <- subcommand("s1")(subcommand1)
//          s2 <- subcommand("s2", "s3")(subcommand2)
          subcommands <- subcommands(subcommand1.asInstanceOf[SubcommandWithCommandLine[Any]], subcommand2.asInstanceOf[SubcommandWithCommandLine[Any]])
        } yield
          if (c eqv Some("illegal"))
            IO.raiseError(IncorrectCommandLineUsage("c cannot be illegal"))
          else
...
"""),
  ...
  -h, --help      Show this help message and exit.
"""),
        Seq("s2", "--help") -> failWithUsageHelpRequested("""Usage: CommandLineParserTest s2 [-h] [-b=PARAM] [COMMAND]
  -b=PARAM     (default: 1).
  -h, --help   Show this help message and exit.
Commands:
  s4
"""),
        ...
        Seq("s2", "-b", "0") -> failCommandExecutionWith[IllegalArgumentException](t => assert(t.getMessage == """s2 failed"""),
          out = Some("s2 running\n"), err = Some("s2 failed")),
        Seq("s2", "-b", "99") -> failCommandExecutionWith[IncorrectCommandLineUsageInSubcommand](t => assert(t.getMessage == "b cannot be 99"),
          out = Some("s2 running\n"), err = Some("""b cannot be 99
Usage: CommandLineParserTest s2 [-h] [-b=PARAM] [COMMAND]
  -b=PARAM     (default: 1).
  -h, --help   Show this help message and exit.
Commands:
  s4
...
Commands:
  ...
  s2, s3
""")),
        Seq("s2", "s4", "-b", "0") -> failCommandExecutionWith[IllegalArgumentException](t => assert(t.getMessage == """s4 failed"""),
          out = Some("s4 running\n"), err = Some("s4 failed")),
        Seq("s2", "s4", "--help") -> failWithUsageHelpRequested("""Usage: CommandLineParserTest s2 s4 [-h] [-b=PARAM]
  -b=PARAM     (default: 1).
  -h, --help   Show this help message and exit.
"""),
        Seq("s2", "s4", "-b", "98") -> failCommandExecutionWith[IncorrectCommandLineUsageInSubcommand](t => assert(t.getMessage == "b cannot be 98"),
          out = Some("s4 running\n"), err = Some("""b cannot be 98
Usage: CommandLineParserTest s2 s4 [-h] [-b=PARAM]
  -b=PARAM     (default: 1).
  -h, --help   Show this help message and exit.
"""))
      )
